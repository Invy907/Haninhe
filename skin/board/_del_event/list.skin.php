...
<div id="link1" class="index-content">
		...
		<div class="content" style="margin:0px 0px 20px 0px;">
			<div class="container">
    				...
    				<div style="margin-bottom:30px;">
    					<?php if ($sca=="進行中のイベント") {?>
					<button class="tablink" onclick="location.href='<?=G5_URL?>/event?sca=進行中のイベント'" style="background-color:#FFF;width:50%;">OPEN</button>
					<button class="tablink" onclick="location.href='<?=G5_URL?>/event?sca=終了イベント'" style="border-bottom:1px #cfcfcf solid;width:50%;border-right:1px #cfcfcf solid;">CLOSED</button>
					<?php } else { ?>
					<button class="tablink" onclick="location.href='<?=G5_URL?>/event?sca=進行中のイベント'" style="border-bottom:1px #cfcfcf solid;width:50%;">OPEN</button>
					<button class="tablink" onclick="location.href='<?=G5_URL?>/event?sca=終了イベント'" style="background-color:#FFF;width:50%;border-right:1px #cfcfcf solid;">CLOSED</button>
					<?php } ?>
				</div>
				<br><br>
			...
			<li><button type="button" class="btn_bo_sch btn_b01 btn" title="検索"><i class="fa fa-search" aria-hidden="true"></i><span class="sound_only">検索</span></button></li>
				...
				<div class="row ma-n12 ma-lg-n30 mt20">
					...
					for ($i=0; $i<count($list); $i++) { 
						$thumb = get_list_thumbnail($board['bo_table'], $list[$i]['wr_id'], 358, 358, false, true);
						if($thumb['src']) {
					...
					?>
					<div class="pa-12 pa-lg-30 col-xs-6  col-sm-4 col-md-4 col-lg-4 col-6" style="margin-bottom:20px;">
						...
						<div style="padding-top:5px;">
							...
							<div style="padding:10px 0px 5px 0px;">
								...
								<div style="padding-top:3px;font-size:0.8em;color:#E74F3D;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;font-family:'Meiryo UI';">
									<a href="#" style="color:#E74F3D;"><?=$list[$i]['wr_1']?></a>
								</div>
							</div>
						</div>
					</div>
	...
	<div style="padding:120px 0px;text-align:center;background-color:#EDE9E6;border-radius:5px;">
		<i class="fa fa-info-circle" aria-hidden="true"></i> イベント情報がありません。
	</div>
	<?php } ?>
			</div>
		</div>
	...
	<div style="padding-top:10px;text-align:center;padding-top:20px;">
		<?php if ($list_href || $write_href) { ?>
			<?php if ($write_href) { ?><a href="<?php echo $write_href ?>" class="btn_submit btn" title="Add NEWS" style="font-weight:normal;"><i class="fa fa-pencil-square" aria-hidden="true"></i> イベント追加</a><?php } ?>
		<?php } ?>
	</div>
	<?php } ?>
