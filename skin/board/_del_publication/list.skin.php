			...
			<li><button type="button" class="btn_bo_sch btn_b01 btn" title="검색"><i class="fa fa-search" aria-hidden="true"></i><span class="sound_only">검색</span></button></li>
				...
				<div class="row ma-n12 ma-lg-n30 mt20">
					...
					for ($i=0; $i<count($list); $i++) { 
						$thumb = get_list_thumbnail($board['bo_table'], $list[$i]['wr_id'], 286, 0, false, true);
						if($thumb['src']) {
					...
					<div class="pa-12 pa-lg-30 col-xs-6  col-sm-3 col-md-3 col-lg-3 col-6" style="margin-bottom:20px;">
						<a href="#" title="<?=$list[$i]['wr_subject']?>">
						...
						<div style="padding-top:5px;">
							<div style="padding:10px 2px 0px 0px;">
								<span style="border:1px solid #999;padding:1px 10px 1px 10px;border-radius:5px;font-size:1.0em;"><?=$list[$i]['wr_3']?></span>
								<div style="float:right;padding:2px 0px 1px 10px;font-size:0.9em;"><?=get_datetime_format($list[$i]['wr_datetime'])?></div>
								<br style="clear:both;">
							</div>
							<div style="padding:10px 0px 5px 0px;">
								<div style="font-size:1.1em;color:#003F77;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;;">
									<a href="#" style="color:#003F77;"><?=$list[$i]['wr_subject']?></a>
								</div>
								<div style="padding-top:3px;font-size:0.8em;color:#E74F3D;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">
									<a href="#" style="color:#E74F3D;">SIZE : <?=$list[$i]['wr_1']?>, PAGE : <?=$list[$i]['wr_2']?></a>
								</div>
								<div style="background-color:#EEE;padding:5px;border-radius:4px;margin-top:15px;height:90px;overflow:hidden;">
									<?php echo cut_str(strip_tags($list[$i]['wr_content']), 100)?>
								</div>
								<div style="text-align:center;padding-top:15px;">
									<button class="button2" onclick="javascript:alert('준비중');">샘플보기</button>
									<button class="button3" onclick="javascript:alert('준비중');">신청하기</button>
								</div>
								<br>
							</div>
						</div>
