...
function latest($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    ...
    if(true){
        ...
        $tmp_write_table = $g5['write_prefix'] . $bo_table; // 게시판 테이블 전체이름
        	if ($options) {
        $sql = " select * from {$tmp_write_table} where wr_is_comment = 0 AND ca_name='{$options}' order by wr_datetime DESC limit 0, {$rows} ";
        } else {
       ...
       //echo $sql."<br>";
        ...
        for ($i=0; $row = sql_fetch_array($result); $i++) {
            ...
            // 썸네일 추가
            if($options && is_string($options) && false) {
...
function latest_news($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options="")
{
    global $g5;
	...
	$table_list = array('info', 'news');
	for ($j=0; $j<count($table_list); $j++) {
		
		$bo_table = $table_list[$j];
		
        $sql = " select * from {$g5['board_table']} where bo_table = '{$bo_table}' ".$tmp_where;
        $board = sql_fetch($sql);
        $bo_subject = get_text($board['bo_subject']);
  	  ...
  	  $tmp_where = "wr_is_comment = 0 AND wr_1=''  AND wr_2 REGEXP '{$options}' ";
  	  $tmp_where = "";
        $sql = " SELECT * FROM {$tmp_write_table} ".$tmp_where." ORDER BY wr_datetime DESC, wr_num LIMIT 0, {$rows} ";
	 ...
	 //echo $sql."<br>";
        ...
        for ($i=0; $row = sql_fetch_array($result); $i++) {
            ...
            $list[] = get_list($row, $board, $latest_skin_url, $subject_len, $bo_table);

		//echo $bo_table."<br>";
		//echo $list[$i]['bo_table']."<br>";
		//echo $list[$i]['wr_id']."<br>";
            $list[$i]['first_file_thumb'] = (isset($row['wr_file']) && $row['wr_file']) ? get_board_file_db($bo_table, $row['wr_id'], 'bf_file, bf_content', "and bf_type in (1, 2, 3, 18) ", true) : array('bf_file'=>'', 'bf_content'=>'');
            //$list[$i]['bo_table'] = $bo_table;
	...
	{
	    $sort_keys[$key] = $value['wr_datetime'];
	}
