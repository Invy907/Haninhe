...
if (!$is_social_password_check && (! (isset($mb['mb_id']) && $mb['mb_id']) || !login_password_check($mb, $mb_password, $mb['mb_password'])) ) {
	...
	// g4_member에서 이전된 계정이 있는지 체크 mb_password가 공백이고 mb_password2가 있는 경우
	if (strlen($mb['mb_password'])==0 && strlen($mb['mb_password2']) > 0) {
		// g4_member 로그인체크
		if (sql_password($mb_password) == $mb['mb_password2']) {
			// mb_password에 입력한 password로 갱신
			$sql = "UPDATE g5_member SET mb_password='".get_encrypt_string($mb_password)."' WHERE mb_id='".$mb_id."' ";
                sql_query($sql);
		} else {
    			run_event('password_is_wrong', 'login', $mb);
    			alert('가입된 회원아이디가 아니거나 비밀번호가 틀립니다.\\n비밀번호는 대소문자를 구분합니다. e01');
		}
	} else {
		// 로그인 실패
    		run_event('password_is_wrong', 'login', $mb);
    		alert('가입된 회원아이디가 아니거나 비밀번호가 틀립니다.\\n비밀번호는 대소문자를 구분합니다. e02');
	}
