...
// ===== メモ：recruit と life_info ページを管理者だけ閲覧可にする =====
// 2025-10-10
if (isset($bo_table) && ($bo_table === 'recruit' || $bo_table === 'life_info')) {

    // 管理者かどうかチェック
    // $is_admin が "super" または "group" の場合、あるいは管理者IDが "admin" の場合は閲覧可能
    if (!($is_admin === 'super' || $is_admin === 'group' || $member['mb_id'] === 'admin')) {
        // 管理者以外はHOMEにリダイレクト
        header("Location: /");
        exit;
    }
